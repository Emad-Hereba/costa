...
export const FormMultiColumnComboBoxVirtualPurchaseProduct = (fieldRenderProps: FieldRenderProps) => {
    const {
        validationMessage,
        touched,
        onFocus,
    ...
    const supplierId = useAppSelector(state => state.orderUi.selectedSupplierId);
    ...
    const columns = [
        ...
        {field: "lastPrice", header: "Last Price", width: "200px"},
    ];
    ...
    const requestData = React.useCallback((skip: number, filter: string) => {
        ...
        agent.Products.getPurchaseProductsLov(params)
            .then((json) => {
                if (json) {
