...
export const FormMultiColumnComboBoxVirtualInventoryItemProduct = (fieldRenderProps: FieldRenderProps) => {
    const {
        validationMessage,
        touched,
        onFocus,
    ...
    const facilityId = useAppSelector(state => state.facilityInventoryUi.selectedFacilityId);
    ...
    const requestData = React.useCallback((skip: number, filter: string) => {
        ...
        agent.Products.getInventoryItemProductsLov(params)
            .then((json) => {
                if (json) {
